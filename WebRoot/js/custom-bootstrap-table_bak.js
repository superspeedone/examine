/**
 * 全局函数对象调用初始化表格
 * 调用方式 $.MyTable.init(tableID, dataURL, paramIDs, setParams, rownums);
 */
;(function($) {
	$.extend({
		MyTable : {
			"init" : function(tableID, dataURL, paramIDs, setParams, rownums){
				return initTable(tableID, dataURL, paramIDs, setParams, rownums);
			}
		}
	});
});
/**
 * 表格初始化 调用方式 initTable(tableID, dataURL, paramIDs, setParams, rownums);
